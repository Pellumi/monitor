...
> **Scope revision (2026-09-15).** Codebase analysis exists to power Tellann's internal assistance, not as a user-facing exploration tool, so the per-view UI above is no longer rendered. The desktop panel shows only a summary (snapshot identity, headline counts, coverage, languages, domains) and, when findings exist, a risk notice listing every risk title with a **Download report** action. The report (`apps/desktop/src/main/codebase-risk-report.ts`) is a PDF covering each risk's severity, description, why it matters, affected code, coupling measurements and blast radius where relevant, remediation steps, evidence, and the analysis's coverage limits. The full graph (entities, relationships, features, architecture, coverage, explanations) is still produced, persisted locally or in the cloud, and served to internal consumers through the `codebaseQuery` IPC and gateway APIs.
