...
export const domainTemplates: Record<string, DomainTemplate> = {
  ECOMMERCE: {
    ...
    states: [
      { name: 'ANONYMOUS', category: 'NAVIGATION', role: 'INITIAL' },
      ...
      { name: 'WISHLIST', category: 'UI', role: 'TERMINAL', terminalKind: 'ALTERNATE' },
      { name: 'CART', category: 'BUSINESS' },
      { name: 'CHECKOUT', category: 'BUSINESS' },
      { name: 'CHECKOUT_SUCCESS', category: 'BUSINESS' },
      { name: 'ORDER_TRACKING', category: 'BUSINESS', role: 'TERMINAL', terminalKind: 'SUCCESS' },
    ],
    transitions: [
  ...
  LMS: {
    ...
    states: [
      ...
      { name: 'LESSON_COMPLETE', category: 'BUSINESS', role: 'TERMINAL', terminalKind: 'SUCCESS' },
    ],
    transitions: [
  ...
  AUTH: {
    ...
    states: [
      { name: 'REGISTER', category: 'BUSINESS', role: 'INITIAL' },
      { name: 'LOGIN', category: 'BUSINESS' },
      { name: 'AUTHENTICATED_HOME', category: 'NAVIGATION' },
      { name: 'LOGOUT', category: 'BUSINESS', role: 'TERMINAL', terminalKind: 'SUCCESS' },
    ],
    transitions: [
      { from: 'REGISTER', to: 'LOGIN', action: 'REGISTRATION_COMPLETE' },
    ...
    edgeCases: [
      ...
      },
      {
        ...
        criticality: 'HIGH',
        confidence: 0.88,
        reason: 'Login must define behavior when the authentication service or network is unavailable.',
      },
      {
        trigger: 'LOGIN',
        name: 'ACCOUNT_LOCKED',
        ...
        criticality: 'HIGH',
        confidence: 0.86,
        reason: 'Repeated invalid credentials should lead to a declared account-lockout path.',
      },
      {
        trigger: 'LOGIN',
        name: 'MFA_REQUIRED',
        ...
        criticality: 'MEDIUM',
        confidence: 0.78,
        reason: 'Privileged or enrolled accounts may require a second authentication factor.',
      },
      {
        trigger: 'LOGIN',
        name: 'PASSWORD_RESET',
  ...
  GENERIC_CRUD: {
    ...
    states: [
      { name: 'LIST_VIEW', category: 'NAVIGATION', role: 'INITIAL' },
      { name: 'DETAIL_VIEW', category: 'BUSINESS' },
      { name: 'CREATE_FORM', category: 'UI' },
      { name: 'CREATE_SUCCESS', category: 'BUSINESS', role: 'TERMINAL', terminalKind: 'SUCCESS' },
      { name: 'EDIT_FORM', category: 'UI' },
      { name: 'UPDATE_SUCCESS', category: 'BUSINESS', role: 'TERMINAL', terminalKind: 'SUCCESS' },
      { name: 'DELETE_CONFIRMATION', category: 'UI', role: 'TERMINAL', terminalKind: 'ALTERNATE' },
    ],
    transitions: [
