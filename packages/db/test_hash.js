...
console.log("Stored hash matches sha256('tellann_' + rawKey):", crypto.createHash('sha256').update("tellann_" + rawKey).digest('hex') === storedHash);
console.log("Hash of 'tellann_' + rawKey:", crypto.createHash('sha256').update("tellann_" + rawKey).digest('hex'));
