...
/** The adapter ids served by this module. */
export const PYTHON_ADAPTER_IDS: FrameworkId[] = PYTHON_DEFINITIONS.map((definition) => definition.id);

/** Whether an adapter id belongs to a Python framework rather than a JavaScript one. */
export function isPythonAdapterId(id: string): boolean {
  return PYTHON_ADAPTER_IDS.includes(id as FrameworkId);
}

/**
 * Repository-relative root of the Python project a Python adapter instruments.
 *
 * The JavaScript adapters answer this from a `package.json` dependency list,
 * which a Python project does not have. Callers that need to know which package
 * owns a file - splitting a Flow's checkpoints across adapters, for one - need
 * the same answer for Python, and getting `null` here would silently drop every
 * Python checkpoint on the floor.
 *
 * Returns `''` for a project at the repository root, and `null` when the
 * framework is not present.
 */
export function pythonAdapterRoot(workspaceRoot: string, id: string): string | null {
  const definition = PYTHON_DEFINITIONS.find((item) => item.id === id);
  if (!definition) return null;
  return projectFor(workspaceRoot, definition)?.relativeRoot ?? null;
}
...
application rather than here. Configuration is read from the process
environment, falling back to the ".env.local" Tellann writes beside this file.
"""
...
#: Read in order; the first file to define a key wins.
ENVIRONMENT_FILES = (".env.local", ".env")

#: Only Tellann's own settings are loaded from those files. This module is not
#: a general dotenv implementation, and quietly defining DJANGO_SETTINGS_MODULE
#: or DEBUG for an application that did not ask for it would change how that
#: application runs.
ENVIRONMENT_PREFIX = "TELLANN_"


def _load_environment_files() -> None:
    """Load Tellann settings from the environment file beside this module.

    Tellann writes the connection settings to ".env.local". A bundler-based
    JavaScript project gets that file loaded for free; Python does not, so a
    server the developer starts by hand - rather than one Tellann launches,
    which is given the values directly - would otherwise see none of it.

    Values already present in the environment always win, so a deployment's own
    configuration is never overwritten by a file in the checkout.
    """
    ...
    for name in ENVIRONMENT_FILES:
        ...
        for raw in content.splitlines():
            ...
            if not separator:
                continue
            key = key.strip()
            if not key.startswith(ENVIRONMENT_PREFIX) or key in os.environ:
                continue
            value = value.strip()
...
#: No placeholder fallback. An application id that is not the real one looks
#: like a working install while every event lands against the wrong
#: application, which is worse than sending nothing at all.
APPLICATION_ID = os.environ.get("TELLANN_APPLICATION_ID")
...
def initialize() -> None:
    ...
    if not APPLICATION_ID:
        # The SDK would disable itself anyway; saying so once turns a silent
        # absence of telemetry into something the developer can act on.
        warnings.warn(
            "Tellann is not configured: TELLANN_APPLICATION_ID is not set and no "
            + repr(ENVIRONMENT_FILES[0])
            + " beside "
            + repr(str(Path(__file__).resolve().parent))
            + " defines it. Telemetry is disabled for this process.",
            RuntimeWarning,
            stacklevel=2,
        )
        return
