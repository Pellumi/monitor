  ...
  try {
    ...
    res.json({
      ...
      boundary: {
        ...
        acceptedEvents: run.progressEvents.filter((event) => event.accepted),
        quarantinedEvents: run.progressEvents.filter((event) => !event.accepted),
      },
      ...
      flowInitialization: run.flowInitialization ? {
        id: run.flowInitialization.id,
        status: run.flowInitialization.status,
        codeReviewReport: run.flowInitialization.codeReviewReport,
        validation: run.flowInitialization.validation,
      } : null,
      flowScan: run.flowScan ? {
        id: run.flowScan.id,
        kind: run.flowScan.kind,
        scannerVersion: run.flowScan.scannerVersion,
        conformanceFindings: run.flowScan.conformanceFindings,
        completedAt: run.flowScan.completedAt,
      } : null,
      drift: run.flowDrift ? {
        id: run.flowDrift.id,
        implementationDiff: run.flowDrift.implementationDiff,
        remediationAlignment: run.flowDrift.remediationAlignment,
        regressions: run.flowDrift.regressions,
        expectedFlowImpact: run.flowDrift.expectedFlowImpact,
      } : null,
...
app.get('/reports/:applicationId/export', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (format === 'html') {
      ...
      <head>
        <meta charset="UTF-8">
        <title>Tellann Report - ${reportData.appName}</title>
      ...
      <body class="p-8 max-w-5xl mx-auto space-y-8 bg-neutral-950 text-neutral-50">
        <div class="flex items-center justify-between border-b border-neutral-800 pb-6">
          ...
          <div class="text-right">
            <span class="text-sm font-semibold uppercase tracking-wider text-blue-400 bg-blue-500/10 px-3 py-1.5 rounded-full border border-blue-500/20">Tellann Coverage Report</span>
          </div>
        </div>
    ...
    if (format === 'pdf') {
      ...
      doc.fillColor('#3b82f6').fontSize(14).text('TELLANN', 50, 200);
      ...
      doc.fillColor('#71717a').fontSize(10).text('Generated by Tellann · Confidential', 50, doc.page.height - 80);
      ...
      for (let i = 0; i < pages.count; i++) {
        ...
        if (i > 0) {
          doc.fillColor('#a1a1aa').fontSize(8)
             .text(`Tellann behavioral QA analysis report · page ${i + 1} of ${pages.count}`, 50, doc.page.height - 35, { align: 'center' });
        }
      }
