  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>
    ...
    *{box-sizing:border-box}
    ...
    <section class="major"><div class="section-label">02 // Previous state</div><h2>Previous state of the project</h2><p>This is the captured state immediately before Tellann wrote the approved instrumentation. File contents and raw diffs remain encrypted locally; the report uses hashes and checkpoint metadata to preserve privacy while supporting comparison and rollback.</p>
      <table><tr><td>BASE REVISION</td><td>${escapeHtml(input.baseRevision ?? "No Git revision recorded")}</td></tr><tr><td>REPOSITORY FINGERPRINT</td><td class="hash">${escapeHtml(input.repositoryFingerprint)}</td></tr><tr><td>CHECKPOINT TYPE</td><td>${escapeHtml(input.checkpoint?.kind ?? "Not recorded")}</td></tr>${input.checkpoint?.previousBranch && input.checkpoint.previousBranch !== input.checkpoint.branch ? `<tr><td>PREVIOUS BRANCH</td><td>${escapeHtml(input.checkpoint.previousBranch)}</td></tr>` : ""}<tr><td>APPLIED ON BRANCH</td><td>${escapeHtml(input.checkpoint?.branch ?? (input.checkpoint?.kind === "GIT_BRANCH" ? "Detached HEAD" : "Local encrypted checkpoint"))}</td></tr><tr><td>WORKTREE DIRTY</td><td>${input.checkpoint?.dirty ? "YES - pre-existing changes were present" : "NO"}</td></tr><tr><td>CHECKPOINT CREATED</td><td>${escapeHtml(input.checkpoint?.createdAt ? new Date(input.checkpoint.createdAt).toLocaleString() : "Not recorded")}</td></tr><tr><td>CHECKPOINT NOTE</td><td>${escapeHtml(input.checkpoint?.reason ?? "No exception recorded")}</td></tr></table>
...
export function renderValidationReportPdf(
  input: ValidationReportInput,
): Promise<Buffer> {
  return renderHtmlToPdf(validationReportHtml(input));
}

/** Prints a self-contained report page to an A4 PDF in a hidden, sandboxed window. */
