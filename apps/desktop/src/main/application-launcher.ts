...
async function resolveExecutable(
  ...
  if (
    ...
    command.args[0] !== "run" ||
    command.args[1] !== command.scriptName ||
    !ALLOWED_SCRIPTS.has(command.scriptName)
  ) {
...
export class LocalApplicationLauncher {
  ...
  get sanitizedOutput(): string {
    return safeOutput(this.output);
  }
  ...
  async start(
    ...
    child.stdout?.on("data", (chunk) => {
      this.output = `${this.output}${String(chunk)}`.slice(-48_000);
    });
    ...
    return {
      pid: child.pid,
      approvalHash: launchApprovalHash(command, workspaceRoot),
    };
  }

  async startPermanent(
    command: LocalLaunchCommand,
    workspaceRoot: string,
