  ...
  return (
    <main className="deskdown-page">
      ...
      <section className="deskdown-section is-inverted" id="handoff">
        ...
        <div className="deskdown-shell deskdown-handoff"><DesktopVisual id="DL-06" label="Browser to Desktop handoff diagram" source="1100 × 360" display="900 × 295" /><div><h3>{validHandoff ? "This browser handoff is ready." : "Already have Tellann Desktop?"}</h3><p>{validHandoff ? "Open Desktop to continue. If it does not open, install the verified release and try again." : "Open the installed application, check the release history, or review update guidance."}</p><div className="deskdown-actions"><a className="deskdown-button is-primary" href={validHandoff ? `tellann://connect?handoff=${encodeURIComponent(validHandoff)}` : "tellann://connect"}>Open Tellann Desktop <span>↗</span></a><Link className="deskdown-button" href={docsLinks.desktopReleases.href}>Check latest release <span>→</span></Link></div></div></div>
      </section>
      ...
      <section className="deskdown-section is-surface">
        ...
        <div className="deskdown-shell"><Link className="deskdown-text-link" href={docsLinks.desktopRequirements.href}>View full system requirements <span>→</span></Link></div>
      </section>
      ...
      <section className="deskdown-resources"><div className="deskdown-shell"><div><p className="deskdown-kicker">Installation support</p><h2>Need help installing?</h2></div><nav aria-label="Desktop installation resources"><a href={`${docsUrl}/desktop/installation`}>Installation guide <span>↗</span></a><Link href={docsLinks.desktopRequirements.href}>System requirements <span>→</span></Link><Link href="/desktop/security">Desktop security <span>→</span></Link><Link href={docsLinks.desktopReleases.href}>Release history <span>→</span></Link><a href={`${docsUrl}/desktop/troubleshooting`}>Troubleshooting <span>↗</span></a></nav></div></section>

      <section className="deskdown-final"><div className="deskdown-shell"><p className="deskdown-kicker">Tellann Desktop</p><h2>Install Tellann Desktop.</h2><p>Connect local development workflows to Tellann from a controlled desktop environment.</p><div className="deskdown-actions"><a className="deskdown-button is-primary" href={installerUrl === "#download-unavailable" ? "#release-unavailable" : installerUrl} aria-disabled={installerUrl === "#download-unavailable"}>Download for Windows <span>→</span></a><Link className="deskdown-button" href={docsLinks.desktopRequirements.href}>View system requirements <span>→</span></Link></div><small id="release-unavailable">Stable release metadata is not yet published.</small></div><div className="deskdown-final-crop"><DesktopVisual id="DL-09" label="Tellann Desktop launch screen crop" source="1400 × 900" display="840 × 540" /></div></section>
    </main>
  );
}
