...
export default function LegacySettingsPage() {
  ...
  const loadBillingData = useCallback(async () => {
    ...
    try {
      const [catalog, nextSubscription, nextInvoices, nextEntitlement] = await Promise.all([
  ...
  async function handleCheckout(plan: Plan) {
    if (!selectedOrgId) return;
    if (subscription && plan.type !== currentPlanType) {
    ...
    try {
      const checkout = await requestJson<{ status: string; checkoutUrl?: string | null; invoiceId: string }>(
        '/api-gateway/billing/subscriptions/checkout',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      <div className="space-y-6">
            ...
            <Card>
              <form onSubmit={handleSaveProfile} className="space-y-6">
                ...
                <div className="flex justify-end border-t border-neutral-800 pt-5">
                  <Button variant="primary" type="submit" disabled={isSavingProfile || !profileDisplayName}>
          ...
          {activeTab === 'security' ? (
            <div className="space-y-6">
              <Card className="space-y-5">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="block">
                    <span className="mb-2 block text-xs font-mono font-medium uppercase tracking-wider text-[#8e9192]">Preferred Sign-In Method</span>
                    <Select value={authMode} onValueChange={(val) => setAuthMode(val as AuthMode)}>
                      ...
                      <SelectContent>
                        <SelectItem value="OTP">{authModeLabels.OTP}</SelectItem>
                        <SelectItem value="PASSWORD">{authModeLabels.PASSWORD}</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
              ...
              <Card className="space-y-5">
                ...
                {mfaEnabled ? (
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-lg border border-emerald-900/60 bg-emerald-950/20 p-4 animate-fade-in">
                    ...
                    <Button variant="danger" type="button" onClick={handleDisableMfa} className="w-full sm:w-auto">
                      Disable MFA
                    </Button>
                  </div>
                ) : showMfaSetup ? (
                  <div className="rounded-lg border border-neutral-800 bg-neutral-950/30 p-5 space-y-5 animate-fade-in">
                    ...
                    <div className="flex justify-end gap-3 border-t border-neutral-800 pt-4">
                      <Button variant="secondary" type="button" onClick={() => { setShowMfaSetup(false); setMfaSetupCode(''); }}>
                        Cancel
                      </Button>
                      <Button variant="primary" type="button" onClick={handleVerifyMfa} disabled={!/^\d{6}$/.test(mfaSetupCode)}>
                        Verify & Activate
                      </Button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 animate-fade-in">
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="block">
                        <span className="mb-2 block text-xs font-mono font-medium uppercase tracking-wider text-[#8e9192]">MFA Platform</span>
                        <Select value={mfaProvider} onValueChange={(val) => setMfaProvider(val as MfaProvider)}>
                          <SelectTrigger className="w-full">
                            <SelectValue placeholder="Select platform...">
                              {mfaProviderLabels[mfaProvider]}
                            </SelectValue>
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <div className="flex items-end">
                        <Button variant="primary" type="button" onClick={() => setShowMfaSetup(true)} className="w-full">
              ...
              <Card className="space-y-5">
                ...
                {canManageTeam && isOrgManager ? (
                  <form onSubmit={handleInviteMember} className="grid gap-4 md:grid-cols-[1fr_160px_auto]">
                    ...
                    <div className="block">
                      <span className="mb-2 block text-xs font-mono font-medium uppercase tracking-wider text-[#8e9192]">Role</span>
                      <Select value={inviteRole} onValueChange={(val) => setInviteRole(val as MemberRole)}>
                        <SelectTrigger className="w-full">
                    ...
                    <div className="flex items-end">
                      <Button variant="primary" type="submit" disabled={isInviting || !inviteEmail.trim()} className="w-full whitespace-nowrap" loading={isInviting}>
                        {!isInviting && <UserPlus className="mr-2 h-4 w-4" />}
                        Send Invite
                      </Button>
                    </div>
                  </form>
                      ...
                      <li key={invitation.id} className="flex flex-col gap-3 px-5 py-4 md:flex-row md:items-center md:justify-between">
                        ...
                        {isOrgManager ? (
                          <Button
                            ...
                            loading={rescindingInvitationId === invitation.id}
                          >
                            Rescind
                          </Button>
                        ) : null}
          ...
          {activeTab === 'preferences' ? (
            <Card className="space-y-6">
              ...
              <div className="flex justify-end border-t border-neutral-800 pt-5">
                <Button variant="primary" type="button" onClick={handleSavePreferences}>
                  Save Preferences
                </Button>
              </div>
            </Card>
          ...
          {activeTab === 'billing' ? (
            <div className="space-y-6">
              <Card className="space-y-5">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="block">
                    <span className="mb-2 block text-xs font-mono font-medium uppercase tracking-wider text-[#8e9192]">Billing Interval</span>
                    <Select value={billingInterval} onValueChange={(val) => setBillingInterval(val as BillingInterval)}>
                      <SelectTrigger className="w-full">
                  ...
                  <div className="rounded-lg border border-neutral-800 bg-neutral-950 px-4 py-3">
                    <div className="text-xs uppercase tracking-wider text-neutral-500">Pricing currency</div>
                    <div className="mt-1 text-sm font-semibold text-white">Prices shown in {billingCurrency}</div>
                    <div className="mt-1 text-xs text-neutral-500">Resolved automatically from the organization billing profile.</div>
                  </div>
                </div>
                <div className="flex flex-wrap gap-3">
                  ...
                  <Button variant="danger" type="button" onClick={handleCancelSubscription} disabled={!subscription || isCancelling} loading={isCancelling}>
                    Cancel Subscription
                  </Button>
                </div>
              </Card>
                        ...
                        <tr>
                          <td colSpan={5} className="p-4">
                            <EmptyState
                              ...
                              description="Invoices will appear after a completed payment."
                            />
                          </td>
                        </tr>
                      )}
                  ...
                  <div className="overflow-hidden rounded-lg border border-neutral-800">
                    ...
                    {auditLogs.length === 0 && !isLoadingAuditLogs ? (
                      <EmptyState
                        ...
                        description="Try changing the filters or check back after organization activity."
                        className="m-4"
                      />
                    ) : (
                      <ul className="divide-y divide-neutral-800">
