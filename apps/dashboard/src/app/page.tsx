...
async function fetchReport(appId: string) {
  const res = await authenticatedFetch(
    `${REPORT_ENGINE}/reports/${appId}/latest`,
  );
  if (!res.ok) {
...
function OverviewContent() {
  ...
  if (!appId) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto my-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-white mb-2 tracking-tight">
            Welcome to Tellann
          </h1>
          <p className="text-[#c4c7c8] text-sm leading-relaxed">
            To start tracking expected states, transitions, and observing
            behavioral gaps, you need to configure your first application.
          </p>
        </div>
        <Link
          href="/onboarding"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <MetricCard
          ...
          value={
            data.coverage.expectedCoverage == null
              ? "N/A"
              : `${data.coverage.expectedCoverage.toFixed(1)}%`
          }
        />
        <MetricCard
          title="State Coverage"
          value={`${data.coverage.stateCoverage.toFixed(1)}%`}
        />
        <MetricCard
          title="Transition Coverage"
          value={`${data.coverage.transitionCoverage.toFixed(1)}%`}
        />
        <MetricCard
          title="Flow Coverage"
          value={`${data.coverage.flowCoverage.toFixed(1)}%`}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-md border border-[#262626] bg-[#131313] p-6">
          ...
          <ul className="space-y-2">
            {data.missingFlows
              .slice(0, 5)
              .map((flow: { path: string[] }, idx: number) => (
                <li
                  key={idx}
                  className="flex flex-col py-2 border-b border-[#262626] last:border-0"
                >
...
function MetricCard({
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#262626] bg-[#131313] px-4 py-5 sm:p-5">
      <dt className="truncate text-xs font-mono font-medium uppercase tracking-wider text-[#8e9192]">
        {title}
      </dt>
      <dd className="mt-1.5 text-2xl font-bold tracking-tight text-white font-mono">
        {value}
      </dd>
    </div>
  );
